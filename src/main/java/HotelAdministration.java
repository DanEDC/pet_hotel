...
public class HotelAdministration {
  ...
  public void registerNewPet(Pet pet) {
    ...
    if (registeredPetsList.size() == 5 && !findPet(pet, reserveList)) {
      reserveList.add(pet);
      System.out.println("The hotel is currently fully booked, " + pet.getAnimalName()
          + " has been added to the reserve list");
    } else if (registeredPetsList.size() == 5 && findPet(pet, reserveList)) {
  ...
  public void hotelCheckOutCheckIn() {
    for (int i = 0; i < registeredPetsList.size(); i++) {
      if (LocalDate.now().equals(registeredPetsList.get(i).getCheckOutDate())) {
        System.out.println(
            registeredPetsList.get(i).getAnimalName() + " has been checked out from the hotel");
        registeredPetsList.remove(registeredPetsList.get(i));
      }
    }
    if (registeredPetsList.size() < 5) {
      ...
      while (count <= 5) {
        for (int i = 0; i < reserveList.size(); i++) {
          if (LocalDate.now().equals(reserveList.get(i).getCheckInDate())) {
            System.out.println(reserveList.get(i).getAnimalName() + " has been checked in to the hotel");
